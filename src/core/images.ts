		...
		await sharp(`${INPUT_IMAGES_PATH}/${file}`)
			...
			.toFile(`${OUTPUT_IMAGES_PATH}/${file}`, (error) => {
				if (error) {
					...
					console.log(
						chalk.green(
							`${emoji.get("framed_picture")} ${file} minified ... [done]`,
						),
					);
				}
			});
...
export async function minImages(): Promise<void> {
	...
	if (JPEG_files.length > 0) await sharpCompress(JPEG_files, 50, "jpeg");
	if (PNG_files.length > 0) await sharpCompress(PNG_files, 50, "png");
	if (WEBP_files.length > 0) await sharpCompress(WEBP_files, 50, "webp");
